...
const SingleDataSelector = ({selection, onSelectionChange, dataTab}: Props) => {
    ...
    const onDataGroupChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const possibleYears = getYears(newSelection);
        if (newSelection.year && !possibleYears.includes(newSelection.year)) {
        ...
        .map(([dataGroup, data]) =>
            <div key={dataGroup}>
                <input
                    ...
                    name="dataGroup" />
                <label className="data-group" htmlFor={dataGroup}>{data.name(selection.normalization)}</label>
